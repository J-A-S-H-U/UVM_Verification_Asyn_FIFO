...
class driv_fifo;
	...
	$display ("-------------INSIDE DRIVER: DRIVE TASK----------------");
	 forever 
		...
		if(!trans.flag)
		begin	
			if(trans.w_en & !vif.full) //When write_enable is high, passing the trans signals to the virtual interface 
			  begin
				vif.w_en <= trans.w_en;
				vif.r_en <= trans.r_en;
			...
			if(trans.r_en & !vif.empty)//When write_enable is high, passing the trans signals to the virtual interface 
			  begin
				vif.r_en <= trans.r_en;
				vif.w_en <= trans.w_en;
				vif.data_in <= trans.data_in;
